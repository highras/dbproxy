...
bool TableManagerBuilder::addTableInfo(TableInfo * ti)
{
	if (ti->splitByRange)
	{
		...
		{
			LOG_ERROR("[Config Error] Table: '%s' for db categroy '%s' with cluster '%s', enable secondary split, but secondary split span is %d",
				ti->tableName.c_str(), ti->databaseCategory.c_str(), ti->cluster.c_str(), ti->seconddarySplitSpan);
			return false;
		}
	}
	else
...
bool TableManagerBuilder::addRangeSplittingInfo(RangeSplittingInfo *rsi)
{
	...
	{
		LOG_ERROR("[Config Error] db category '%s' with cluster '%s', split database index %d, index type is config error.",
			rsi->databaseCategory.c_str(), rsi->cluster.c_str(), rsi->index);
		return false;
	}

	RangeTaskHint hint(*rsi);

	auto iter = _constructureInfo->_rangeSplittingInfos.find(hint);
	...
	else
	{
		...
		_constructureInfo->_rangeSplittingInfos[hint] = tmpList;
	}
	return true;
}
...
bool TableManagerBuilder::init_step5_dbCollection_to_dbTaskQueues(TableManagerPtr oldTableManager)
{
	...
	for (auto& dbPair: _constructureInfo->_dbCollection)
	{
		...
		auto it = _constructureInfo->_dbTaskQueues.find(dbPair.second->master_id);
...
bool TableManagerBuilder::init_step6_tableSplittingInfos_dbTaskQueues_to_tableTaskQueues()
{
	for (auto& tsiPair: _constructureInfo->_tableSplittingInfos)
	{
		...
		{
			LOG_ERROR("[Config Error] Database for split table %s, table number %d with cluster %s is duplicated.",
				tsiPair.first.tableName.c_str(), tsiPair.first.tableNumber, tsiPair.first.cluster.c_str());
			return false;
		}
		...
		{
			LOG_ERROR("[Config Error] Split table: %s, number %d, cluster %s, the host master database's id %d is not found.",
				tsi->tableName.c_str(), tsi->tableNumber, tsi->cluster.c_str(), tsi->serverId);
			return false;
		}
	}
	return true;
...
bool TableManagerBuilder::init_step7_rangeSplittingInfos_dbTaskQueues_to_rangedTaskQueues()
{
	for (auto& rsiPair: _constructureInfo->_rangeSplittingInfos)
	{
		...
		for (RangeSplittingInfo* rsi: rsiPair.second)
		{
			...
			if (configError)
			{
				LOG_ERROR("[Config Error] db category '%s' with cluster '%s', split database index %d is config error.",
					rsiPair.first.databaseCategory.c_str(), rsiPair.first.cluster.c_str(), rsi->index);
				return false;
			}
			...
			if (!dtq)
			{
				LOG_ERROR("[Config Error] db category '%s', %s split database index %d with cluster '%s' is not found.",
					rsiPair.first.databaseCategory.c_str(), (rsi->indexType ? ((rsi->indexType == 1) ? "odd" : "even") : ""),
					rsi->index, rsiPair.first.cluster.c_str());
				return false;
			}
			...
			_usedTaskQueues.insert(dtq);	//-- May add a function into class RangeDatabaseNode to do all above things.
		}
		...
		for (size_t i = 0; i < vec.size(); i++)
		{
			if (vec[i]->oddEvened)
			{
				if (vec[i]->arr[0] == NULL)
				{
					LOG_ERROR("[Config Error] db category '%s', cluster '%s', odd split database index %d is unconfiged.",
						rsiPair.first.databaseCategory.c_str(), rsiPair.first.cluster.c_str(), i);
					return false;
				}
				if (vec[i]->arr[1] == NULL)
				{
					LOG_ERROR("[Config Error] db category '%s', cluster '%s', even split database index %d is unconfiged.",
						rsiPair.first.databaseCategory.c_str(), rsiPair.first.cluster.c_str(), i);
					return false;
				} 
			}
			else
			{
				if (vec[i]->arr[0] == NULL)
				{
					LOG_ERROR("[Config Error] db category '%s', cluster '%s', split database index %d is unconfiged.",
						rsiPair.first.databaseCategory.c_str(), rsiPair.first.cluster.c_str(), i);
					return false;
				}
			}
