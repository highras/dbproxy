...
void ConfigDatabase::addMasterDBClient(int server_id, const std::string& host, int port, const std::string& bizUuser, const std::string& bizPwd, const std::string& adminUser, const std::string& adminPwd, int timeout)
{
	MySQLClientPtr client(new MySQLClient(host, port, adminUser, adminPwd, "", timeout));
	_masterInstances[server_id] = client;

	InstanceNode node{host, std::to_string(port), std::to_string(timeout), bizUuser, bizPwd};
