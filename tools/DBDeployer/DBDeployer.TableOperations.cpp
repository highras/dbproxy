...
void DBDeployer::showAllTables(const std::string& cluster)
{
	std::string sql("select * from table_info where cluster = '");
	sql.append(cluster).append("'");

	showTables(sql);
}

void DBDeployer::showRangeTables(const std::string& cluster)
{
	std::string sql("select id, table_name, range_span, database_category, secondary_split, secondary_split_span, hint_field from table_info where cluster = '");
	sql.append(cluster).append("' and split_type = 1");

	showTables(sql);
}

void DBDeployer::showHashTables(const std::string& cluster)
{
	std::string sql("select id, table_name, table_count, hint_field from table_info where cluster = '");
	sql.append(cluster).append("' and split_type = 0");

	showTables(sql);
}

bool DBDeployer::tableSplitType(const std::string& tableName, const std::string& cluster, bool& splitByRange, int& hashTableCount)
{
	if (_configDB.dbName().empty())
	{
		cout<<"Please laod or create config database first."<<endl;
	...
	sql.append(tableName).append("' and cluster = '").append(cluster).append("'");
	...
	{
		cout<<"Cannot find table "<<tableName<<" in cluster "<<cluster<<endl;
		return false;
	}
...
bool DBDeployer::getRangeTableInfo(const std::string& tableName, const std::string& cluster, std::string& databaseCategory,
...
void DBDeployer::dropHashTable(const std::string& tableName, const std::string& cluster, int tableCount)
{
	std::string loadTablesSQL("select table_number, server_id, database_name from split_table_info where table_name = '");
	loadTablesSQL.append(tableName).append("' and cluster = '").append(cluster).append("'");
...
void DBDeployer::dropRangeTable(const std::string& tableName, const std::string& cluster)
{
	bool secondarySplit;
	std::string databaseCategory;
	int64_t span, secondarySpan;

	if (!getRangeTableInfo(tableName, cluster, databaseCategory, span, secondarySplit, secondarySpan))
		return;
...

	{
		...
		sql.append(databaseCategory).append("' and cluster = '").append(cluster).append("'");
...
void DBDeployer::dropTable(const std::string& tableName, const std::string& cluster)
{
	bool splitByRange;
	int hashTableCount;
	if (tableSplitType(tableName, cluster, splitByRange, hashTableCount) == false)
		return;

	if (splitByRange)
		dropRangeTable(tableName, cluster);
	else
		dropHashTable(tableName, cluster, hashTableCount);
}
...
bool DBDeployer::insertHashTableInfo(const std::string& tableName, const std::string& cluster, const std::string& database, int tableNumber, int serverId)
{
	std::ostringstream ossSQL;
	ossSQL<<"insert into split_table_info (table_name, table_number, cluster, server_id, database_name) values ('";
	ossSQL<<tableName<<"', "<<tableNumber<<", '"<<cluster<<"', "<<serverId<<", '"<<database<<"')";
...
void DBDeployer::createHashTable(const std::string& tableName, const std::string& createSQL, const std::string& hintField, int tableCount, const std::string& cluster, const std::string& targetDatabase, int databaseCount)
{
	...
	//-- fill table_info
	{
		...
		oss<<"insert into table_info (table_name, cluster, split_type, table_count, hint_field) values ('";
		oss<<tableName<<"', '"<<cluster<<"', 0, "<<tableCount<<", '"<<hintField<<"')";
	...
	if (tableCount == 1)
	{
		...
		if (!insertHashTableInfo(tableName, cluster, targetDatabase, 0, deployServerId))
			return;
	}
	else
	{
		for (int i = 0; i < tableCount; i++)
		{
			...
			if (!insertHashTableInfo(tableName, cluster, databaseName, i, deployServerId))
				return;
		}
	}
