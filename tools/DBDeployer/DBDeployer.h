...
class DBDeployer
{
	...
	bool tableSplitType(const std::string& tableName, const std::string& cluster, bool& splitByRange, int& hashTableCount);
	bool getRangeTableInfo(const std::string& tableName, const std::string& cluster, std::string& databaseCategory,
		int64_t& span, bool& secondarySplit, int64_t& secondarySpan);
	int fetchSecondarySplitBase();
	int64_t fetchGlobalRangeSpan();
	void dropHashTable(const std::string& tableName, const std::string& cluster, int tableCount);
	void dropHashTable(const std::string& tableName, const std::vector<std::string>& infos, bool split);
	void dropRangeTable(const std::string& tableName, const std::string& cluster);

	bool insertHashTableInfo(const std::string& tableName, const std::string& cluster, const std::string& database, int tableNumber, int serverId);
...
public:
	...
	//-- table operations
	void showAllTables(const std::string& cluster);
	void showRangeTables(const std::string& cluster);
	void showHashTables(const std::string& cluster);
	void dropTable(const std::string& tableName, const std::string& cluster);
	bool checkTableNameInCreateSQL(const std::string& tableName, const std::string& createSQL, bool silence = false);
	void createHashTable(const std::string& tableName, const std::string& createSQL, const std::string& hintField, int tableCount, const std::string& cluster, const std::string& targetDatabase, int databaseCount);
