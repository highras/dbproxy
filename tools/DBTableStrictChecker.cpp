...
std::set<int64_t> getHintIds(std::shared_ptr<TCPClient> client, const std::string& tableName, const std::string& cluster)
{
	std::cout<<"Query interface 'allSplitHintIds' ..."<<std::endl;

	FPQWriter qw(2, "allSplitHintIds");
...
void checkTable(std::shared_ptr<TCPClient> client, int64_t hintId, const std::string &tableName, const std::string& cluster,
...
void hashTableCheck(std::shared_ptr<TCPClient> client, const std::string& tableName, const std::string& cluster, FPAReader &splitInfoAnswerReader)
{
	...
	std::set<int64_t> hintIds = getHintIds(client, tableName, cluster);
	...
	for (int i = 0; i < tableCount; i++)
		checkTable(client, i, tableName, cluster, descSQL, standardTableIdx, std::to_string(i), hintIds, columsDefinitions, errorInfos);

	printErrorInfos(errorInfos, hintIds);
}


void rangeTableCheck(std::shared_ptr<TCPClient> client, const std::string& tableName, const std::string& cluster, FPAReader &splitInfoAnswerReader)
{
		...
		if (oddEvenRanges.find(i) != oddEvenRanges.end())
		{
			if (secondarySplit)
			{
				...
				for (int j = 0; j < secondaryTableCountInFirstSpan; j++)
				{
					...
					checkTable(client, rangeHintId + j * secondarySplitSpan    , tableName, cluster, descSQL, standardTableIdx, 
						indexInfo + addedInfo + " (even-sub)", hintIds, columsDefinitions, errorInfos);

					checkTable(client, rangeHintId + j * secondarySplitSpan + 1, tableName, cluster, descSQL, standardTableIdx, 
			...
			else
			{
				std::string indexInfo;
				indexInfo.append(" in range ").append(std::to_string(i));
				checkTable(client, rangeHintId    , tableName, cluster, descSQL, standardTableIdx, indexInfo + " (even-sub)", hintIds, columsDefinitions, errorInfos);
				checkTable(client, rangeHintId + 1, tableName, cluster, descSQL, standardTableIdx, indexInfo + " (odd-sub)" , hintIds, columsDefinitions, errorInfos);
			}
		}
		else
		{
			...
			else
			{
				std::string indexInfo;
				indexInfo.append(" in range ").append(std::to_string(i));
				checkTable(client, rangeHintId, tableName, cluster, descSQL, standardTableIdx, indexInfo, hintIds, columsDefinitions, errorInfos);
			}
		}
	}
...
void check(const std::string& host, int port, const std::string& tableName, const std::string& cluster)
{
	...
	if (checkException(ar))
	{
		...
		if (ar.wantBool("splitByRange"))
			rangeTableCheck(client, tableName, cluster, ar);
		else
			hashTableCheck(client, tableName, cluster, ar);
	}
